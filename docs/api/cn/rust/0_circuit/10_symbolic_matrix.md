...
除上述函数外，模块还导出一组就地将门矩阵写入符号矩阵的底层函数，主要用于矩阵分块构造和自定义门实现。

| 函数 | 说明 |
| --- | --- |
| `apply_standard_gate_to_matrix(matrix, gate, bits, params)` | 将标准门就地作用到指定比特上。 |
| `apply_gate_to_matrix(matrix, gate, bits)` | 将符号门矩阵就地作用到指定比特上。 |
| `apply_gate_to_matrix_num(matrix, gate, bits)` | 将数值门矩阵就地作用到指定比特上。 |
| `apply_single_qubit_gate(matrix, gate, bit)` / `apply_single_qubit_gate_num(matrix, gate, bit)` | 单比特门就地作用，分别接收符号门矩阵与数值门矩阵。 |
| `apply_two_qubit_gate(matrix, gate, b0, b1)` / `apply_two_qubit_gate_num(matrix, gate, b0, b1)` | 双比特门就地作用。 |
| `apply_general_gate(matrix, gate, bits)` / `apply_general_gate_num(matrix, gate, bits)` | 任意比特数的门就地作用。 |
| `apply_symbolic_permutation_gate(matrix, permutation, bits)` / `apply_numeric_permutation_gate(matrix, permutation, bits)` | 就地作用置换型门。 |
| `apply_symbolic_diagonal_gate(matrix, diagonal, bits)` / `apply_numeric_diagonal_gate(matrix, diagonal, bits)` | 就地作用对角型门。 |
...
例如，如果线路中的量子比特编号为 `0` 和 `2`，显式顺序可以写为 `[0, 2]` 或 `[2, 0]`，二者会得到不同轴顺序下的矩阵表示。
