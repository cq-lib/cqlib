...
对于使用频率较高的几种模板，`ansatz` 模块提供了一组快捷构造函数，用于按模板名直接创建实例。它们返回的仍是上表中的模板对象，可以继续使用 builder 方法调整配置。

| 函数 | 返回 | 说明 |
| --- | --- | --- |
| `real_amplitudes(num_qubits, reps, entanglement)` | `TwoLocal` | 由 `RY` 旋转层与纠缠层交替组成的模板。 |
| `efficient_su2(num_qubits, reps, entanglement)` | `TwoLocal` | 由 `RY`、`RZ` 旋转层与纠缠层交替组成的模板。 |
| `zz_feature_map(num_qubits, reps, entanglement)` | `ZZFeatureMap` | 二阶 ZZ feature map。 |
| `pauli_feature_map(num_qubits, reps, paulis, entanglement)` | `PauliFeatureMap` | 按给定 Pauli string 模板构造的 feature map。 |

其中 `paulis` 是 `(PauliString, String)` 列表，前者指定相互作用类型，后者是该 Pauli 项的标签。
...
| `Auto { steps }` | 自动选择策略：对易时使用 exact，非对易时使用一阶 Trotter，步数为 `steps`。 |
