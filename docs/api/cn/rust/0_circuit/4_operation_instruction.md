...
`Instruction` 提供一组不依赖 `Debug` 输出、也不随展示格式变化的稳定标识，适合在编译 pass、序列化和缓存键中使用。

| 方法 | 返回 | 说明 |
| --- | --- | --- |
| `name()` | `String` | 具体操作名或用户自定义门标签。与 `instruction_type()` 不同，它能区分同类别下的不同门。 |
| `instruction_type()` | `&'static str` | 指令类别名：`"standard"`、`"mcgate"`、`"unitary"`、`"circuit"`、`"directive"`、`"classical_data"`、`"classical_control"`、`"delay"`。 |

`name()` 对各变体的取值来源：标准门取门名，多控制门取多控制门名，酉门取标签，复合门取门名，指令与经典操作取各自的操作名，`Delay` 固定为 `"delay"`。

类别判定用一组常量谓词，全部在编译期求值：

| 谓词 | 说明 |
| --- | --- |
| `is_standard()` | 是否为标准门指令。 |
| `is_mcgate()` | 是否为多控制门指令。 |
| `is_unitary()` | 是否为用户自定义酉门指令。 |
| `is_circuit_gate()` | 是否为复合门指令。 |
| `is_directive()` | 是否为非酉指令。 |
| `is_classical_data()` | 是否为经典数据操作。 |
| `is_classical_control()` | 是否为结构化控制流。 |
| `is_delay()` | 是否为延迟指令。 |
...
`Directive` 还提供稳定的操作名接口：

```rust
pub const fn name(self) -> &'static str
```

分别返回 `"barrier"`、`"measure"`、`"reset"`。
...
`Operation::standard_gate()` 在操作为标准门时返回对应标准门定义，否则返回 `None`。
...
| `name()` | 返回具体操作名或用户自定义门标签。 |
| `instruction_type()` | 返回稳定类别名；构造层控制流固定为 `"classical_control"`，其余委托给内部 `Instruction`。 |
