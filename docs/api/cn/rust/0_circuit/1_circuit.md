...
| `used_symbols()` | `IndexSet<String>` | 返回可执行 IR 实际引用的符号名集合。 |
| `uses_symbol(symbol)` | `bool` | 判断指定符号是否被可执行 IR 引用。 |
...
| `delay(qubit, delay)` | 在指定量子比特上插入空闲时间。 |
| `multi_control(instruction, controls, targets, params)` | 构造多控制标准门。 |
...
| `remove_operation(index)` | 删除单条顶层操作并返回被删除的操作。 |
| `remove_operations(indices)` | 删除多条顶层操作，按原始下标升序返回被删除的操作。 |
