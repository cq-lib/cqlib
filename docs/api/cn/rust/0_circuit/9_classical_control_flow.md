...
| `ty()` | 返回表达式的静态类型。 |
| `kind()` | 返回底层 AST 节点。 |
| `vars()` | 返回表达式读取的全部可变经典变量集合。 |
| `values()` | 返回表达式读取的全部不可变经典值集合。 |
| `collect_vars(out)` | 将读取的经典变量写入给定的集合。 |
| `collect_values(out)` | 将读取的经典值写入给定的集合。 |
| `remap_classical_ids(var_map, value_map)` | 按映射表重映射表达式引用的经典句柄。 |
| `simplified()` | 返回结构化简后的表达式。 |
| `is_bool_true()` / `is_bool_false()` | 判断是否为 `Bool` 字面量。 |
| `is_bit_true()` / `is_bit_false()` | 判断是否为 `Bit` 字面量。 |

---

## 表达式 AST 类型

`ClassicalExpr` 的底层节点由下列类型描述，它们都在 `cqlib_core::circuit` 模块根可见。
...
| 类型 | 说明 |
| --- | --- |
| `ClassicalExprKind` | 表达式 AST 节点枚举，覆盖变量读取、字面量、运算、比较、类型转换、位操作。 |
| `ClassicalExprNode` | AST 节点，携带表达式的静态类型与节点内容。 |
| `ClassicalUnaryOp` / `ClassicalBinaryOp` | 一元与二元逻辑运算算子。 |
| `ClassicalCompareOp` | 比较算子。 |
| `ClassicalCast` | 显式类型转换算子。 |
...
### 4. `switch` 与 `SwitchBuilder`

`switch(target, build)` 的回调参数是 `SwitchBuilder`，用于按顺序登记各 case 与可选的 default 分支。

| 方法 | 说明 |
| --- | --- |
| `value(value, body)` | 登记一个精确匹配的 case 分支。 |
| `default(body)` | 登记默认分支；重复登记会返回错误。 |
