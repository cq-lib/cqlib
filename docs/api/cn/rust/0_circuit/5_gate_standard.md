...
| 方法 | 返回 | 说明 |
| --- | --- | --- |
| `name()` | `&'static str` | 返回门的稳定操作名，例如 `"H"`、`"RX"`、`"CX"`。 |
...
`matrix()` 返回该标准门的局部数值矩阵。`params` 需按 `self.num_params()` 提供参数，其中任意一项为 `NaN` 或无穷大时返回 `InvalidParameterValue`；参数个数少于 `num_params()` 属于调用错误。
...
`inverse()` 返回当前标准门的反门枚举及变换后的参数列表。每个标准门都有可表示的反门，因此该接口对标准门总是返回 `Some`。`params` 需按 `num_params()` 提供足量参数，参数个数不足属于调用错误。
...
| `X2P` / `X2M` | 互为反门 |
| `Y2P` / `Y2M` | 互为反门 |
| `XY2P` / `XY2M` | 互为反门，参数保持不变 |
| `Phase(λ)` / `GPhase(λ)` | 参数取负 |
| `RXX` / `RYY` / `RZZ` / `RZX` | 参数取负 |
| `RXY(θ, φ)` | `RXY(-θ, φ)`，`φ` 保持不变 |
| `FSIM(θ, φ)` | `FSIM(-θ, -φ)` |
| `XY(θ)` | `XY(π + θ)` |
| `U(θ, φ, λ)` | `U(-θ, -λ, -φ)`，后两个参数互换 |
