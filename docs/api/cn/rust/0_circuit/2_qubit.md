...
对于可能为负数或可能超出 `u32` 范围的整数类型，应使用 `TryFrom`：有符号类型 `i8`、`i16`、`i32`、`isize`，以及无符号类型 `usize`。其中 `usize` 与 `isize` 在 64 位系统上可以表示超出 `u32` 的值，转换失败时不会被静默截断，而是返回错误。
...
`Qubit` 的编号是逻辑标识。在线路中，量子比特的实际顺序由 `Circuit` 保存的量子比特列表决定；在矩阵或态矢量中，轴顺序也由相关转换接口的量子比特顺序约定决定。
