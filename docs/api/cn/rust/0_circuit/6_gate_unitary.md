...
`UnitaryMatrix` 是 `UnitaryGate` 的原始矩阵表示枚举，可通过 `matrix_repr()` 读取。
...
| 变体 | 说明 |
| --- | --- |
| `Numeric(Array2<Complex<f64>>)` | 数值矩阵，仅适用于非参数化的酉门定义。 |
| `Symbolic(SymbolicMatrix)` | 符号矩阵，其自由符号按门的 `matrix_params` 顺序进行位置映射。 |
...
此外，使用数值矩阵定义时，`num_params` 必须为 `0`，因为该门的矩阵已经完全确定，不需要在应用时额外传入位置参数。矩阵元素必须为有限值，且矩阵必须是酉矩阵，即 `U†U` 在容差范围内为单位矩阵；不满足时会返回错误。
...
其中，`params` 指定门在应用时传入的位置参数如何映射到符号矩阵中的变量名。也就是说，`params` 不只是一个说明性列表，而是定义了参数绑定顺序。调用 `matrix_for_params()` 或将该门追加到线路时，传入的第 `i` 个参数会绑定到 `params[i]` 对应的符号名。`params` 的长度必须等于 `num_params`，且符号矩阵中出现的自由符号都必须已在 `params` 中声明。
...
| Circuit-backed 定义 | 按内部电路 `used_symbols()` 顺序绑定参数，再对内部线路调用 `circuit_to_matrix()`。 |
