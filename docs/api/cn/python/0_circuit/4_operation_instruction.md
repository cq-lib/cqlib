...
| 静态方法 | 说明 |
| --- | --- |
| `Instruction.from_name(name)` | 按标准门名称创建标准门指令，名称匹配不区分大小写；名称无法匹配任何标准门时抛 `ValueError`。 |
| `Instruction.from_standard_gate(gate)` | 从 `StandardGate` 创建标准门指令；`gate` 必须是不带参数的未绑定定义，否则抛 `CircuitError`。 |
| `Instruction.from_mc_gate(gate)` | 从 `MCGate` 创建多控制门指令；其基础门同样必须是不带参数的未绑定定义，否则抛 `CircuitError`。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| `name` | `str` | 指令的可读名称。标准门指令使用规范化的大写名称，例如 `"H"`、`"CX"`；`Directive` 与 `delay` 指令使用小写名称，例如 `"barrier"`、`"measure"`、`"delay"`。 |
...
| `name` | `str` | 指令名称；包裹控制流时返回结构名，例如 `"if"`、`"while"`。 |
...
| `label` | `str \| None` | 可选标签。 |
| `name` | `str` | 操作名称；控制流操作返回结构名，例如 `"if"`。 |
| `num_qubits` | `int` | 本次操作作用的量子比特数量。 |
| `num_params` | `int` | 本次操作携带的参数数量。 |
| `instruction_type` | `str` | 指令类别，取值与 `Instruction.instruction_type` 一致。 |
| `is_standard` / `is_mcgate` / `is_unitary` / `is_circuit_gate` / `is_directive` / `is_classical_data` / `is_classical_control` / `is_delay` | `bool` | 与 `Instruction` 同名的类别判定属性。 |
| `matrix()` | `np.ndarray` | 返回该操作对应的酉矩阵；携带符号参数或为控制流操作时抛 `ValueError`。 |
