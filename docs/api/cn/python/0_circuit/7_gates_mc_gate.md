...
调用时需要保证参数数量与 `num_params` 一致：显式传入的 `params` 长度不等于 `num_params`，或者未传入 `params` 而门上的参数尚未全部绑定为具体数值时，都会抛出 `CircuitError`。传入的数值参数必须为有限值，否则抛出 `ParameterError`。
...
调用 `inverse()` 时要求门上的参数已经完整绑定：已绑定参数数量不等于 `num_params` 时抛出 `CircuitError`；基础门本身不可取逆时抛出 `RuntimeError`。
...
- 支持 `==` 与 `hash`，比较与哈希同时考虑控制结构、基础门与已绑定参数，因此 `MCGate` 可以作为字典键或集合元素。
- 支持 `copy`、`deepcopy`。
- `repr` 可直接求值重建原门：无参数时形如 `MCGate(2, StandardGate.X)`，带参数时形如 `MCGate(1, StandardGate.RX(1.5))`。
