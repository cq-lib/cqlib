...
| `symbols` | `list[str]` | 线路中所有自由符号名称。该列表是一个稳定的符号登记表，可能包含已不再被可执行 IR 引用的符号。 |
| `used_symbols` | `list[str]` | 当前可执行 IR 实际引用的自由符号名称。 |
...
| `operation(index)` | 返回指定位置的 `ValueOperation`；索引越界时抛 `IndexError`。 |
| `remove_operation(index)` | 移除指定位置的操作，并返回被移除的 `ValueOperation`；索引越界时抛 `IndexError`。 |
| `remove_operations(indices)` | 批量移除多个位置的操作，并返回被移除的 `ValueOperation` 列表；索引越界时抛 `IndexError`。若待移除的测量结果仍被 `store` 引用，则抛 `CircuitError`。 |
| `__len__()` | 返回线路中的操作数量。 |
| `__getitem__(index)` | 支持使用正向或负向索引读取操作。 |
| `uses_symbol(symbol)` | 判断当前可执行 IR 是否引用了指定符号，返回 `bool`。 |
| `var(ty)` | 分配一个由当前线路持有的可变经典变量，`ty` 为 `ClassicalType`。 |
| `store(target, value)` | 将经典表达式 `value` 写入经典变量 `target`。 |
| `dag()` | 返回当前线路对应的有向无环图表示 `CircuitDag`。 |
| `validate()` | 校验线路结构、经典句柄、作用域和控制流不变量。 |
| `depth(recurse=False)` | 计算线路深度。 |
| `set_global_phase(phase)` | 设置线路全局相位。 |


`depth()` 用于估算线路深度。在线性量子线路中，深度通常表示按尽早调度（ASAP）方式排列后，最长量子比特路径上的操作层数。普通门和普通指令通常贡献一层；`barrier` 会约束其覆盖量子比特上的重排；`CircuitGate` 默认可视为一个不透明操作。当线路中包含 `if`、`while`、`for`、`switch`、`break` 或 `continue` 等控制流结构时，深度计算需要额外处理分支和循环语义：默认的 `recurse=False` 不会展开这些结构，此时若线路中存在控制流，`depth()` 会抛出 `CircuitError`；传入 `recurse=True` 则递归展开分支与循环体，返回估算深度。
...
| `phase(qubit, lambda)` | `float \| Parameter` | 相位门 `P(lambda)`。 |
| `u(qubit, theta, phi, lambda)` | `float \| Parameter` | 通用单量子比特门。 |
...
与 `inverse()`、`decompose()` 等返回新线路的方法不同，`compose()` 就地修改当前线路，并返回 `None`。
...
在 `while_()` 与 `for_uint()` 的回调体中，可以调用 `break_loop()` 提前跳出当前循环或 `switch` 分支，或调用 `continue_loop()` 跳过本轮剩余部分。这两者只允许出现在对应的控制流体内部，否则会抛 `CircuitError`。
