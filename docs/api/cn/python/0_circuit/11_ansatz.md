...
模块导出的全部名称如上，包括 `real_amplitudes`、`efficient_su2`、`zz_feature_map` 和 `pauli_feature_map` 四个便捷函数，以及 `EvolutionInfo`。
...
`validate()` 会在配置不合法时抛出 `ValueError`，例如量子比特数量、门类型、拓扑或参数设置相互不匹配；`build_circuit()` 在同样的情形下同样抛出 `ValueError`。各模板对象都支持 `copy`、`deepcopy`，并带有便于调试的 `repr` 与 `str`。
