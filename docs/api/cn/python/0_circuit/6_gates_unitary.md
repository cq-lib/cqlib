...
`matrix()` 返回由 `with_matrix()` 附加的数值矩阵。如果该门尚未定义，或改用 `with_symbolic_matrix()`、`with_circuit()` 定义，则不存在可直接返回的数值矩阵，此时抛 `CircuitError`。

`matrix_for_params(params)` 用于求出该门在给定参数下的数值矩阵，支持数值矩阵、符号矩阵和冻结线路三种定义方式；`params` 按 `matrix_params` 给出的顺序提供。
...
`UnitaryGate` 同时实现数组协议，因此也可以通过 `np.array(gate)` 或 `np.asarray(gate)` 直接取得矩阵表示。
