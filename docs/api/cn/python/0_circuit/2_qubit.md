...
`Qubit(index)` 用于创建一个逻辑量子比特句柄。`index` 必须是非负整数，内部会以无符号整数形式保存。可接受的取值范围为 `0` 到 `4294967295`（即 `2**32 - 1`）：传入负整数或大于该上界的整数会触发 `QubitError`，传入非整数（例如浮点数或字符串）则触发 `TypeError`。

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| `index` | `int` | 逻辑量子比特编号，取值需在 `0` 到 `4294967295` 之间。 |
...
`Qubit` 的编号是逻辑标识，不要求从 `0` 开始连续排列。例如，`Qubit(10)` 和 `Qubit(20)` 可以共同构成一条双比特线路。
...
需要注意的是，逻辑编号不一定等同于矩阵或态矢量中的轴顺序。对于 `Circuit([Qubit(10), Qubit(20)])`，线路内部只包含两个量子比特，矩阵维度为 `4 × 4`。默认情况下，矩阵转换会按照线路中保存的 `qubits` 顺序解释量子比特。
