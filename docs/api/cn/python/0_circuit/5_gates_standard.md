...
`all()` 按标准门枚举顺序返回全部内置标准门定义，列表中的每一项都是未绑定参数的门的定义。

`from_name(name)` 按名称查找标准门定义，匹配不区分大小写；名称无法匹配任何标准门时抛 `ValueError`。
...
传入的 `params` 长度必须与门的 `num_params` 一致，否则抛 `CircuitError`；已绑定的参数若无法求值为常量，则抛 `ParameterError`。参数化门在未绑定任何参数时直接调用 `matrix()` 同样抛 `CircuitError`。
...
当门实例包含参数时，`inverse()` 会按照该门的逆变换规则处理参数：例如 `StandardGate.RZ(0.5).inverse()` 返回 `StandardGate.RZ(-0.5)`。该能力常用于构造逆线路、uncompute 结构、门级测试和编译优化验证。

参数化门在未绑定全部参数时调用 `inverse()` 会抛 `CircuitError`；如果该门本身不可逆，则抛 `RuntimeError`。
