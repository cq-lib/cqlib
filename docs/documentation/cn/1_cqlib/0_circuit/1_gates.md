...
`cqlib.circuit` 提供了丰富的量子门与线路指令，用于构造从基础量子线路到参数化算法模块的各类量子程序。本篇将详细介绍 Cqlib 中量子门与指令的基本用法，包括标准门、参数门、多比特门、多控制门、自定义酉门、子线路门以及测量、重置、屏障等非酉指令。
...
在量子线路分析、算法验证和编译转换过程中，门的矩阵表示是理解其数学作用的重要依据。Cqlib 支持对标准门和多控制门直接计算矩阵，以便您检查门的维度、验证门的酉性，以及分析门对量子态的具体作用。
...
- `Barrier` 不改变量子态，可视为自身的逆；`Measure` 和 `Reset` 不具备普通酉逆操作。
...
当算法需要使用内置标准门以外的酉操作时，可以通过 `UnitaryGate` 定义自定义酉门。自定义酉门适用于表示算法中的特殊 oracle、问题相关变换、硬件专用门、已知矩阵形式的量子操作。
...
除数值矩阵外，自定义酉门也可以通过符号矩阵定义。符号矩阵适合描述带参数的门族，可以先定义门的符号形式，再从不同线路或不同位置传入具体参数值。这种方式常用于参数化 oracle、可调相位门、算法模板和符号验证场景。
...
除普通量子门外，量子线路中还可能包含测量、重置、屏障和延迟等特殊指令。这类指令通常无法对应一个普通的酉矩阵，因此在 Cqlib 中统一通过 `Directive` 表示。
...
常用的非酉指令可以直接通过 `Circuit` 提供的接口添加到线路中：
...
需要注意的是，非酉指令会影响线路的数学表示和后续处理方式：

- `barrier` 本身不改变量子态，仅用于表达编译约束。因此，当线路中不包含其他非酉操作时，带有 `barrier` 的线路仍可用于矩阵转换。
- `measure`、`measure_into` 和 `measure_bits` 会引入量子测量与经典结果，因此线路不再能用单一酉矩阵完整表示。
- `reset` 会将量子比特重新初始化到固定状态，属于非酉操作，同样不能通过普通酉矩阵描述。
