...
- `instruction`：操作对应的 `ValueInstruction`，可以表示普通量子门、非酉指令或经典控制流结构；
- `qubits`：该操作作用的量子比特列表；
...
在线路创建完成后，您可以向 `Circuit` 中持续追加量子门、复合门、非酉指令或低层操作对象。Cqlib 提供了多种追加操作的方式，以适应不同的使用需求。
...
每条 `Circuit` 都包含一个 `global_phase` 属性，用于记录线路整体的全局相位。全局相位不改变各计算基态的测量概率分布，但它是量子线路数学表示的一部分，在矩阵对比、线路等价性判断、编译重写以及某些相位敏感的算法分析中具有重要意义。
