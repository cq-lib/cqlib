...
`CompileConfig` 本身只包含 `mode`、`target`、`resource_policy` 三项；其余参数在 `compile()` 入口或 `CompileTarget` 的构造方法上设置。

| 参数 | 设置位置 | 作用 |
|------|---------|------|
| `mode` | `CompileConfig` | `normal` / `enhanced` |
| `target` | `CompileConfig` | 目标约束，取值见 [`CompileTarget`](../../../../api/cn/python/4_compile/1_compiler.md) |
| `resource_policy` | `CompileConfig` | 分解阶段辅助比特策略 |
| `device` | `compile()` | 可用比特、拓扑、native gates 与可选标定；单独使用时选择严格 `Device` 目标 |
| `target_basis` | `compile()` | 显式目标门集；与 `device` 同时使用时选择 `TopologyBasis`，不承诺设备原生兼容，也不运行 Pareto beam |
| `initial_layout` | `compile()` 或 `CompileTarget` 的构造方法 | 跳过自动布局，直接用给定映射做 SABRE 路由 |
| `seed` | `compile()` 或 `CompileTarget` 的构造方法 | 启发式布局/路由随机试次 |
