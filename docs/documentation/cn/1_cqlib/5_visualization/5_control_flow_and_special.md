...
除了基础门和常规参数化线路，Cqlib 的线路图也可以显示动态控制流、非酉指令和自定义门。遇到这类线路时，读图重点不只是门顺序，还包括控制流块覆盖了哪些 qubit lane、分支在哪里结束、循环中是否存在 `break` / `continue`，以及特殊门标签是否保留了算法含义。
