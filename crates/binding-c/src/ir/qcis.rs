...
#[unsafe(no_mangle)]
pub extern "C" fn qcis_loads(source: *const c_char) -> *mut CCircuit {
    if source.is_null() {
    ...
    match ir::qcis_loads(source) {
        Ok(circuit) => Box::into_raw(Box::new(CCircuit { inner: circuit })),
        Err(_) => std::ptr::null_mut(),
    }
}

/// Load a QCIS file into a circuit.
/// Returns a heap-allocated `CCircuit*`, or NULL on error.
#[unsafe(no_mangle)]
pub extern "C" fn qcis_load(path: *const c_char) -> *mut CCircuit {
    if path.is_null() {
        return std::ptr::null_mut();
    }
    ...
    match ir::qcis_load(path) {
        Ok(circuit) => Box::into_raw(Box::new(CCircuit { inner: circuit })),
        Err(_) => std::ptr::null_mut(),
    }
}

/// Dump a circuit to a QCIS string.
/// Returns a heap-allocated C string (caller must free with `cqlib_string_free`),
/// or NULL on error.
#[unsafe(no_mangle)]
pub extern "C" fn qcis_dumps(circuit: *const CCircuit) -> *mut c_char {
    if circuit.is_null() {
        return std::ptr::null_mut();
    }
    let circuit = unsafe { &(*circuit).inner };
    match ir::qcis_dumps(circuit) {
...
/// Dump a circuit to a QCIS file.
/// Returns 0 on success, or a negative error code on failure.
#[unsafe(no_mangle)]
pub extern "C" fn qcis_dump(circuit: *const CCircuit, path: *const c_char) -> i32 {
    ...
    match ir::qcis_dump(circuit, path) {
